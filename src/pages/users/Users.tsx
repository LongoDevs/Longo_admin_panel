  ...
  const changeProviderStatus = async (providerId: string, newStatus: string) => {
    try {
      ...
      setProviders(providers.map(provider => 
        provider._id === providerId ? { ...provider, status: newStatus as any } : provider
      ));
              ...
              {selectedProviders.length > 0 && (
                ...
                >
                  ...
                  <button 
                    ...
                    onClick={() => {
                      const providersToDelete = providers.filter(provider => selectedProviders.includes(provider._id));
                      
                      setProviders(providers.filter(provider => !selectedProviders.includes(provider._id)));
                  ...
                  <input
                    ...
                    onChange={() => {
                      if (selectedProviders.length === currentProviders.length) {
                        ...
                        setSelectedProviders(currentProviders.map(p => p._id));
                      }
                    }}
                  />
                </th>
              ...
              <AnimatePresence>
                {currentProviders.map((provider, index) => (
                  <motion.tr 
                    ...
                    className={selectedProviders.includes(provider._id) ? 'bg-yellow-900/20' : ''}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                      <button
                        onClick={() => setShowDropdown(showDropdown === provider._id ? null : provider._id)}
                        className="text-gray-400 hover:text-white"
                      >
                        <MoreHorizontal className="h-5 w-5" />
                      </button>
                      {showDropdown === provider._id && (
